...
## HSA and HRR are collections of zipcodes
...
load(paste0(DataPath,"EtDT2012.RData")) ## loading referral and ACS data set
...
DT1 <- DT[which(zipcode %in% ZipHsaHrr$zipcode12),] 
# remove some zipcodes not belonging to any HSA, which is defined based on the zip code
rm(DT)
...
## the parallel here isnot efficient, because here the every terminal needs to copy the data
# which takes a lot of time. I choosed to do this by hand, distributed on 40 terminals, each 
# handle about 10,000 rows
library(foreach)
library(doParallel)
...
for( hsa in unique(NpiHsaHrr$Hsanum) ){
  ...
  refer <- Et[which (V1 %in% npis),]
  refer <- refer[which ( V2 %in% npis),]
